# @monigo/tokens

## 0.3.0

## 0.2.0

### Patch Changes

- First alpha release of @monigo/svelte with building-block components, page components, and the composed MonigoPortal. The hosted customer portal at platform/portal is refactored to consume this package.

## 0.1.0

### Minor Changes

- Initial alpha release: design tokens, CSS theme files, createTheme helper, portal API client with full endpoint surface, per-feature state stores, Intl-based formatters, and English message catalog.
